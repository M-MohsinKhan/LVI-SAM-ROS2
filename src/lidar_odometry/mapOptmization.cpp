...
rclcpp::Time pre_lidar_time(0, 0, RCL_ROS_TIME);
...
public:
    ...
    rclcpp::Publisher<sensor_msgs::msg::PointCloud2>::SharedPtr pubCornerSurround;
    rclcpp::Publisher<sensor_msgs::msg::PointCloud2>::SharedPtr pubSurfSurround;
    ...
    rclcpp::Subscription<nav_msgs::msg::Odometry>::SharedPtr subvinsOdom;
    std::deque<nav_msgs::msg::Odometry> vinsQueue;
    ...
    : ParamServer("lvi_sam_mapOptimization", options)
    {
        ...
        pubKeyPoses = this->create_publisher<sensor_msgs::msg::PointCloud2>(PROJECT_NAME + "/lidar/mapping/trajectory", qos());
        pubLaserCloudSurround = this->create_publisher<sensor_msgs::msg::PointCloud2>(PROJECT_NAME + "/lidar/mapping/map_global", qos());
        pubCornerSurround = this->create_publisher<sensor_msgs::msg::PointCloud2>(PROJECT_NAME + "/lidar/mapping/map_corner", qos());
        pubSurfSurround = this->create_publisher<sensor_msgs::msg::PointCloud2>(PROJECT_NAME + "/lidar/mapping/map_surf", qos());
        pubOdomAftMappedROS = this->create_publisher<nav_msgs::msg::Odometry>(PROJECT_NAME + "/lidar/mapping/odometry", qos()); //原来的全局里程计
        pubPath = this->create_publisher<nav_msgs::msg::Path>(PROJECT_NAME + "/lidar/mapping/path", qos());

        subLaserCloudInfo = this->create_subscription<lvi_sam::msg::CloudInfo>(
            PROJECT_NAME + "/lidar/feature/cloud_info", qos(),
        ...
        subLoopInfo = this->create_subscription<std_msgs::msg::Float64MultiArray>(
            PROJECT_NAME + "/vins/loop/match_frame", qos(),
            std::bind(&mapOptimization::loopHandler, this, std::placeholders::_1));

        pubHistoryKeyFrames = this->create_publisher<sensor_msgs::msg::PointCloud2>(PROJECT_NAME + "/lidar/mapping/loop_closure_history_cloud", qos());
        pubIcpKeyFrames = this->create_publisher<sensor_msgs::msg::PointCloud2>(PROJECT_NAME + "/lidar/mapping/loop_closure_corrected_cloud", qos());
        pubLoopConstraintEdge = this->create_publisher<visualization_msgs::msg::MarkerArray>(PROJECT_NAME + "/lidar/mapping/loop_closure_constraints", qos());

        pubRecentKeyFrames = this->create_publisher<sensor_msgs::msg::PointCloud2>(PROJECT_NAME + "/lidar/mapping/map_local", qos());
        pubRecentKeyFrame = this->create_publisher<sensor_msgs::msg::PointCloud2>(PROJECT_NAME + "/lidar/mapping/cloud_registered", qos());
        pubCloudRegisteredRaw = this->create_publisher<sensor_msgs::msg::PointCloud2>(PROJECT_NAME + "/lidar/mapping/cloud_registered_raw", qos());
     ...
     * @param vinsMsg
     */
    void vinsHandler(const nav_msgs::msg::Odometry::SharedPtr vinsMsg)
    {
        vinsQueue.push_back(*vinsMsg);
        if (first_lidar_keypose)
        {
            duration = this->now() - pre_lidar_time;
        }
        if (duration.seconds() > 1.0)
        {
            timeLastProcessing = rclcpp::Time(vinsQueue.back().header.stamp).seconds();
            // 1.计算点云的先验位姿 (通过imu或者vins odom)
            ...
            cout << "using vins:" << this->now().seconds() << endl;
        }
    }
    void updateInitialGuess_vins()
    {
        ...
        //获取的是imu的位姿
        tf2::Quaternion q_odom_imu;
        tf2::fromMsg(odomCur.pose.pose.orientation, q_odom_imu);
        tf2::Transform t_odom_imu = tf2::Transform(q_odom_imu, tf2::Vector3(odomCur.pose.pose.position.x, odomCur.pose.pose.position.y, odomCur.pose.pose.position.z));
        Eigen::Vector3d ypr = extRot.eulerAngles(2, 1, 0);
        tf2::Quaternion q_lidar_imu;
        q_lidar_imu.setRPY(ypr.z(), ypr.y(), ypr.x());
        tf2::Transform t_lidar_imu = tf2::Transform(q_lidar_imu, tf2::Vector3(extTrans.x(), extTrans.y(), extTrans.z()));
        tf2::Transform t_odom_lidar = t_odom_imu * (t_lidar_imu.inverse());
        tf2::Quaternion q_zero;
        q_zero.setRPY(0, 0, 0);
        tf2::Quaternion q_odom_lidar = q_zero * (t_odom_lidar.getRotation());
        // t_odom_lidar = tf2::Transform(q_odom_lidar, tf2::Vector3(t_odom_lidar.getOrigin().x(), t_odom_lidar.getOrigin().y(), t_odom_lidar.getOrigin().z()));
        tf2::Matrix3x3 ml(q_odom_lidar);
    ...
    void transformUpdate() //优化后具有时延 所以用当前时刻的imu信息来进行优化
    {
        if (cloudInfo.imu_available == true)
        {
            if (std::abs(cloudInfo.imu_pitch_init) < 1.4)
            {
                ...
                tf2::Matrix3x3(transformQuaternion.slerp(imuQuaternion, imuWeight)).getRPY(rollMid, pitchMid, yawMid); //四元数之间球面插值得到的结果
...
int main(int argc, char **argv)
{
    ...
    auto mapOpt = std::make_shared<mapOptimization>(options);

    RCLCPP_INFO(mapOpt->get_logger(), "\033[1;32m----> Lidar Map Optimization Started.\033[0m");

    std::thread loopDetectionthread(&mapOptimization::loopClosureThread, mapOpt.get());
    std::thread visualizeMapThread(&mapOptimization::visualizeGlobalMapThread, mapOpt.get());
