...
public:
    ...
    : ParamServer("lvi_sam_imuPreintegration", options)
    {
        ...
        subOdometry = this->create_subscription<nav_msgs::msg::Odometry>(
            PROJECT_NAME + "/lidar/mapping/odometry", qos_odom(),
            std::bind(&IMUPreintegration::odometryHandler, this, std::placeholders::_1));

        pubImuOdometry = this->create_publisher<nav_msgs::msg::Odometry>("odometry/imu", qos());
        pub_latest_odometry = this->create_publisher<nav_msgs::msg::Odometry>("odometry/test", qos());
        pubImuPath = this->create_publisher<nav_msgs::msg::Path>(PROJECT_NAME + "/lidar/imu/path", qos());
    ...
    void imuHandler(const sensor_msgs::msg::Imu::SharedPtr imuMsg)
    {
        ...
        if (imuTime - last_path_time > 0.1)
        {
            ...
            //队列中的头与对列尾时间差大于3s
            while (!imuPath.poses.empty() && abs(rclcpp::Time(imuPath.poses.front().header.stamp).seconds() - rclcpp::Time(imuPath.poses.back().header.stamp).seconds()) > 3.0)
        ...
        tf_broadcaster_->sendTransform(odom_2_baselink);
    }
};
...
int main(int argc, char **argv)
{
    ...
    RCLCPP_INFO(ImuP->get_logger(), "\033[1;32m----> Lidar IMU Preintegration Started.\033[0m");
