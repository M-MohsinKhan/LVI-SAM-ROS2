                             ...
                             const std_msgs::msg::Header &header)
{
    ...
    {
        RCLCPP_WARN(rclcpp::get_logger("estimator"), "calibrating extrinsic param, rotation movement is needed");
        if (frame_count != 0)
        {
    ...
    else
    {
        ...
        if (failureDetection())
        {
            RCLCPP_ERROR(rclcpp::get_logger("estimator"), "VINS failure detection!");
            failure_occur = 1;
            clearState();
            setParameter();
            RCLCPP_ERROR(rclcpp::get_logger("estimator"), "VINS system reboot!");
            return;
        }
...
bool Estimator::initialStructure()
{
    // Lidar initialization
    {
        ...
        for (int i = 0; i <= WINDOW_SIZE; i++)
        {
                ...
                all_image_frame[rclcpp::Time(Headers[i].stamp).seconds()].reset_id != all_image_frame[rclcpp::Time(Headers[0].stamp).seconds()].reset_id)
            {
                ...
                RCLCPP_INFO(rclcpp::get_logger("estimator"), "Lidar initialization info not enough.");
                break;
            }
        }
    ...
    //check imu observibility
    {
        ...
        //RCLCPP_WARN(rclcpp::get_logger("estimator"), "IMU variation %f!", var);
        if(var < 0.25)
        {
            RCLCPP_INFO(rclcpp::get_logger("estimator"), "Trying to initialize VINS, IMU excitation not enough!");
            //return false;
        }
    }
    // global sfm
...
bool Estimator::visualInitialAlign()
{
    ...
    if(!result)
    {
        RCLCPP_INFO(rclcpp::get_logger("estimator"), "solve gravity failed, try again!");
        return false;
    }
...
bool Estimator::failureDetection()
{
    ...
    {
        RCLCPP_ERROR(rclcpp::get_logger("estimator"), "VINS little feature %d!", f_manager.last_track_num);
        //return true;
    }
    if (Bas[WINDOW_SIZE].norm() > 2.5)
    {
        RCLCPP_ERROR(rclcpp::get_logger("estimator"), "VINS big IMU acc bias estimation %f, restart estimator!", Bas[WINDOW_SIZE].norm());
        return true;
    }
    if (Bgs[WINDOW_SIZE].norm() > 1.0)
    {
        RCLCPP_ERROR(rclcpp::get_logger("estimator"), "VINS big IMU gyr bias estimation %f, restart estimator!", Bgs[WINDOW_SIZE].norm());
        return true;
    }
    if (Vs[WINDOW_SIZE].norm() > 30.0)
    {
        RCLCPP_ERROR(rclcpp::get_logger("estimator"), "VINS big speed %f, restart estimator!", Vs[WINDOW_SIZE].norm());
        return true;
    }
    Vector3d tmp_P = Ps[WINDOW_SIZE];
    if ((tmp_P - last_P).norm() > 5.0)
    {
        RCLCPP_ERROR(rclcpp::get_logger("estimator"), "VINS big translation, restart estimator!");
        return true;
    }
    if (abs(tmp_P.z() - last_P.z()) > 1)
    {
        RCLCPP_ERROR(rclcpp::get_logger("estimator"), "VINS big z translation, restart estimator!");
        return true;
    }
    ...
    if (delta_angle > 50)
    {
        RCLCPP_ERROR(rclcpp::get_logger("estimator"), "VINS big delta_angle, moving too fast!");
        //return true;
    }
    return false;
}
