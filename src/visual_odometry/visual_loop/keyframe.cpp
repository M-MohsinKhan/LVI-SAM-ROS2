...
rclcpp::Node::SharedPtr KeyFrame::node_ = nullptr;
...
bool KeyFrame::findConnection(KeyFrame* old_kf)
{
	...
	if ((int)matched_2d_cur.size() > MIN_LOOP_NUM)
	{
        ...
        if ((int)matched_2d_cur.size() > MIN_LOOP_NUM)
        {
        	if (pub_match_img->get_subscription_count() > 0)
            {
            	int gap = 10;
    	    	...
    	    	auto msg = cv_bridge::CvImage(std_msgs::msg::Header(), "bgr8", loop_match_img).toImageMsg();
