...
public:
    ...
    DepthRegister(std::shared_ptr<rclcpp::Node> node_in) : node_(node_in)
    {
        // messages for RVIZ visualization
        pub_depth_feature = node_->create_publisher<sensor_msgs::msg::PointCloud2>(PROJECT_NAME + "/vins/depth/depth_feature", 5);
        pub_depth_image = node_->create_publisher<sensor_msgs::msg::Image>(PROJECT_NAME + "/vins/depth/depth_image", 5);
        pub_depth_cloud = node_->create_publisher<sensor_msgs::msg::PointCloud2>(PROJECT_NAME + "/vins/depth/depth_cloud", 5);
    ...
    sensor_msgs::msg::ChannelFloat32 get_depth(const builtin_interfaces::msg::Time &stamp_cur, const cv::Mat &imageCur,
                                          ...
                                          const vector<geometry_msgs::msg::Point32> &features_2d)
    {
        ...
        {
            // RCLCPP_ERROR(node_->get_logger(), "image no tf");
            return depth_of_point;
        }
        ...
        *depth_cloud_local = *depth_cloud_local_filter2;
        publishCloud(pub_depth_cloud, depth_cloud_local, rclcpp::Time(stamp_cur), "vins_body_ros");
        ...
        publishCloud(pub_depth_feature, features_3d_sphere, rclcpp::Time(stamp_cur), "vins_body_ros");
